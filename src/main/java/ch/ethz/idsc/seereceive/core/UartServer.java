...
public class UartServer {
	...
	public static UartServer create(UartClientInterface myUartClientInterface) {
		...
		try {
			...
			Logger.getGlobal().log(Level.SEVERE, "failed to launch UartServer " + myUartClientInterface.getPort() + " "
					+ myUartClientInterface.getBaud() + " " + myException.getMessage() + "\n");
			return null;
		}
		return myUartServer;
	}
	...
	private void connect() throws Exception {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				Logger.getGlobal().log(Level.INFO, "UartServer open at port " + myUartClientInterface.getPort()
						+ " with baud " + myUartClientInterface.getBaud() + "\n");
				while (isLaunched)
					try {
